...
from src.crader.providers.embedding import DummyEmbeddingProvider, OpenAIEmbeddingProvider
...
def get_chunk_graph(chunk_id: str):
    ...
    for out in outgoing:
        ...
        if t_row:
            ...
            edges.append({"source": chunk_id, "target": target_id, "relation": out["relation"]})
    ...
    for inc in incoming:
        ...
        if s_row:
            ...
            edges.append({"source": source_id, "target": chunk_id, "relation": inc["relation"]})
    ...
    with storage.connector.get_connection() as conn:
        ...
        for p in parents:
            ...
            if p_row:
                ...
                edges.append({"source": chunk_id, "target": pid, "relation": "child_of"})
        ...
        for c in children:
            ...
            if c_row:
                ...
                edges.append({"source": cid, "target": chunk_id, "relation": "child_of"})
                ...
                nodes.append(
                    {
                        ...
                        "metadata": row[
                            "metadata"
                        ],  # navigator handles enrichment if needed, but here we just pass raw
                        "type": "center",
                    }
                )
        ...
        for ref in impact_data:
            ...
            with storage.connector.get_connection() as conn:
                ...
                if s_row:
                    ...
                    edges.append({"source": ref["source_id"], "target": chunk_id, "relation": ref["relation"]})
    ...
    try:
        ...
        def _process_tree(parent_id, children_dict):
            ...
            for child_id, child_data in children_dict.items():
                ...
                edges.append({"source": parent_id, "target": child_id, "relation": child_data.get("type", "calls")})
...
def get_chunk_neighbors(chunk_id: str):
    ...
    try:
        ...
        if prev_chunk:
            ...
            edges.append({"source": str(prev_chunk["id"]), "target": chunk_id, "relation": "previous"})
        ...
        if next_chunk:
            ...
            edges.append({"source": chunk_id, "target": str(next_chunk["id"]), "relation": "next"})
...
def chat_agent(req: ChatRequest):
    ...
    return StreamingResponse(agent.stream_chat(req.message, req.thread_id), media_type="application/x-ndjson")
...
def get_chunk_parent(chunk_id: str):
    ...
    try:
        ...
        if parent:
            ...
            edges.append({"source": chunk_id, "target": str(parent["id"]), "relation": "child_of"})

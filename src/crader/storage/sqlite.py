...
class SqliteGraphStorage(GraphStorage):
    ...
    def update_repository_status(self, repo_id: str, status: str, commit_hash: str = None):
        now = datetime.datetime.utcnow().isoformat()
        if commit_hash:
            self._cursor.execute("UPDATE repositories SET status = ?, last_commit = ?, updated_at = ? WHERE id = ?", (status, commit_hash, now, repo_id))
        else:
            self._cursor.execute("UPDATE repositories SET status = ?, updated_at = ? WHERE id = ?", (status, now, repo_id))
        self._conn.commit()
    ...
    def delete_previous_data(self, repo_id: str, branch: str):
        try:
            ...
            self._cursor.execute("DELETE FROM edges WHERE source_id IN (SELECT n.id FROM nodes n JOIN files f ON n.file_id = f.id WHERE f.repo_id = ?)", (repo_id,))
            self._cursor.execute("DELETE FROM nodes WHERE file_id IN (SELECT id FROM files WHERE repo_id = ?)", (repo_id,))
    ...
    def add_files(self, files: List[Any]):
        ...
        for f in files:
            ...
            sql_batch.append((d['id'], d['repo_id'], d.get('commit_hash', ''), d['file_hash'], d['path'], d['language'], d['size_bytes'], d['category'], d['indexed_at']))
        if sql_batch:
    ...
    def add_nodes(self, nodes: List[Any]):
        ...
        for n in nodes:
            ...
            b_start = d['byte_range'][0]; b_end = d['byte_range'][1]
    ...
    def add_search_index(self, search_docs: List[Dict[str, Any]]):
        ...
        if sql_batch:
            self._cursor.executemany("INSERT OR REPLACE INTO nodes_fts (node_id, file_path, semantic_tags, content) VALUES (?, ?, ?, ?)", sql_batch)
            self._conn.commit()
    ...
    def get_nodes_cursor(self, repo_id: str = None, branch: str = None) -> Generator[Dict[str, Any], None, None]:
        ...
        """
        params = []
        if repo_id: base_query += " AND f.repo_id = ?"; params.append(repo_id)
    ...
    def find_chunk_id(self, file_path: str, byte_range: List[int], repo_id: str = None) -> Optional[str]:
        ...
        if repo_id: sql += " AND f.repo_id = ?"; params.append(repo_id)
    ...
    def get_files_bulk(self, file_paths: List[str], repo_id: str = None) -> Dict[str, Dict[str, Any]]:
        ...
        for i in range(0, len(unique_paths), 900):
            ...
            params = list(batch)
            if repo_id: sql += " AND repo_id = ?"; params.append(repo_id)
            self._cursor.execute(sql, params)
            if self._cursor.description:
                cols = [d[0] for d in self._cursor.description]
                for row in self._cursor: result[row[0]] = dict(zip(cols, row))
        return result
    ...
    def get_contents_bulk(self, chunk_hashes: List[str]) -> Dict[str, str]:
        ...
        for i in range(0, len(chunk_hashes), 900):
            ...
            for row in self._cursor: result[row[0]] = row[1]
        return result
    ...
    def get_incoming_definitions_bulk(self, node_ids: List[str]) -> Dict[str, List[str]]:
        ...
        for i in range(0, len(node_ids), 900):
            ...
            ph = ",".join(["?"] * len(batch))
            self._cursor.execute(f"SELECT target_id, metadata_json FROM edges WHERE target_id IN ({ph}) AND relation_type='calls'", batch)
    ...
    def get_context_neighbors(self, node_id: str) -> Dict[str, List[Dict[str, Any]]]:
        res = {"parents": [], "calls": []}
        self._cursor.execute("SELECT t.id, t.file_path, t.start_line, e.metadata_json, t.metadata_json FROM edges e JOIN nodes t ON e.target_id = t.id WHERE e.source_id = ? AND e.relation_type = 'child_of'", (node_id,))
        for row in self._cursor:
            res["parents"].append({"id": row[0], "file_path": row[1], "start_line": row[2], "edge_meta": json.loads(row[3] or "{}"), "metadata": json.loads(row[4] or "{}")})
        self._cursor.execute("SELECT t.id, t.file_path, e.metadata_json FROM edges e JOIN nodes t ON e.target_id = t.id WHERE e.source_id = ? AND e.relation_type IN ('calls', 'references') LIMIT 15", (node_id,))
        for row in self._cursor:
    ...
    def get_neighbor_chunk(self, node_id: str, direction: str = "next") -> Optional[Dict[str, Any]]:
        ...
        row = self._cursor.fetchone()
        if row: return {"id": row[0], "start_line": row[1], "end_line": row[2], "chunk_hash": row[3], "content": row[4], "metadata": json.loads(row[5] or "{}")}
        return None

    def get_incoming_references(self, target_node_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        self._cursor.execute("SELECT s.id, s.file_path, s.start_line, e.relation_type, e.metadata_json FROM edges e JOIN nodes s ON e.source_id = s.id WHERE e.target_id = ? AND e.relation_type IN ('calls', 'references', 'imports', 'instantiates') ORDER BY s.file_path, s.start_line LIMIT ?", (target_node_id, limit))
        results = []
        for row in self._cursor:
            m = json.loads(row[4] or "{}")
            results.append({"source_id": row[0], "file": row[1], "line": row[2], "relation": row[3], "context_snippet": m.get("description", "")})
        return results

    def get_outgoing_calls(self, source_node_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        self._cursor.execute("SELECT t.id, t.file_path, t.start_line, e.relation_type, e.metadata_json FROM edges e JOIN nodes t ON e.target_id = t.id WHERE e.source_id = ? AND e.relation_type IN ('calls', 'instantiates', 'imports') ORDER BY t.file_path, t.start_line LIMIT ?", (source_node_id, limit))
        results = []
        for row in self._cursor:
            m = json.loads(row[4] or "{}")
            results.append({"target_id": row[0], "file": row[1], "line": row[2], "relation": row[3], "symbol": m.get("symbol", "")})
        return results

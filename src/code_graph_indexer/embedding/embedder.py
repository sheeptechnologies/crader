...
class CodeEmbedder:
    ...
    def run_indexing(self, snapshot_id: str, batch_size: int = 32, yield_debug_docs: bool = False) -> Generator[Dict[str, Any], None, None]:
        ...
        for node in nodes_cursor:
            ...
            if len(current_batch) >= batch_size:
                saved_docs = self._process_and_save(current_batch, snapshot_id, debug=yield_debug_docs)
    ...
    def _process_and_save(self, nodes: List[Dict], snapshot_id: str, debug: bool = False) -> List[Dict]:
        needed_hashes = set()
        node_ids = []
        ...
        #  Fetch definizioni dal grafo per RAG Enriched
        definitions_map = {}
        if hasattr(self.storage, 'get_incoming_definitions_bulk'):
        ...
        for node in nodes:
            ...
            # [RESTORED] Incoming Definitions (Calls/Symbols)

...
def mock_storage():
    ...
    # Graph traversal
    storage.get_context_neighbors.return_value = {
        'parents': [],
        'children': []
    }
        ...
        with patch('crader.indexer.GitVolumeManager') as mock_git, \
            ...
            with patch('crader.indexer.PooledConnector'), \
                ...
                indexer = CodebaseIndexer(
                    ...
                    branch="main",
                    db_url="postgresql://mock:5432/db"
                )
...
class TestGoWorkflow:
    ...
    def test_index_go_repository(self, mock_storage):
        ...
        with patch('crader.indexer.GitVolumeManager') as mock_git, \
            ...
            mock_parser.return_value.parse_file.return_value = (
                [Mock(id="chunk_go_1", content="func main() {}")],
                []
            )
...
class TestUserScenarios:
    ...
    def test_find_authentication_code(self, mock_storage, mock_embedding_provider):
        ...
        assert len(results) > 0
        assert any(keyword in results[0].content.lower() 
                  for keyword in ['session', 'auth', 'login'])
    ...
    def test_hybrid_search(self, mock_storage, mock_embedding_provider):
        ...
        mock_storage.search_vectors.return_value = [
            {'id': 'v1', 'content': 'request handler', 'score': 0.9}
        ]
        mock_storage.search_fts.return_value = [
            {'id': 'f1', 'content': 'request context', 'score': 0.8}
        ]
